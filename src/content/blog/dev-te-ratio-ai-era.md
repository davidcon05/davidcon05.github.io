...
takeaway: "AI gave devs an efficiency gain validation never got, and headcount can't close it. What can: capturing the human judgment about which journeys matter, in a form that runs without a human present."
status: "PUBLISHED"
order: 8
featured: true
featuredOrder: 1
...
And that's the narrow version of the problem. The broader one is that validation isn't a task AI is slow at — it's a task AI cannot currently do at all, and the reason is worth being precise about.

Watch any of the current AI tools that claim to drive an app. They click around randomly. Not maliciously, not stupidly — they have no basis for choosing one path over another, so they explore. A model handed a screen has no idea which of the eleven tappable things on it a real person would reach for, because that answer doesn't live in the screen. It lives in the person.

How I navigate an app is the sum of everything I've done before opening it. A colleague ten years older moves through it differently than one ten years younger. Someone who joined the team last month goes looking in different places than someone who's been on the product for three years. Someone with sharper eyes, or duller ones, or a tremor, or a screen reader, takes an entirely different route to the same outcome. Every one of those is a real user, and every one of those journeys is a real thing to validate. A model has no history and no body. It cannot simulate any of them, so it substitutes coverage-by-flailing and calls the tokens it burned an effort.

The same limit shows up everywhere validation actually happens. It can't look at a screen and tell you the spacing is wrong against the design, or that the tap target is too small, or that the contrast fails in a way the audit tool didn't catch. And it can't be handed regression and produce an artifact anybody should believe — not because it won't generate one, but because a regression report you can't trace back to deliberate decisions about what mattered is just a document.
...
This is also the case for UI tests specifically, and it's worth separating them from "automation" generally. A UI test isn't a robot poking at the app. It's a journey somebody chose deliberately, because they knew which paths a real user takes and which branches actually carry risk — encoded once and then run identically forever. That's the difference between guided validation and random clicking: the coverage map is tailored to *this* app by someone who understands who uses it. The judgment happens once, up front, by a human. The execution is deterministic and costs nothing to repeat.

Which is the actual answer to the AI-can't-navigate problem. You don't wait for a model sophisticated enough to intuit user behavior. You capture the human judgment you already have, in a form that runs without a human present.
...
My own answer leans toward automation, but not automation as it's usually pitched — more coverage bolted onto the existing process. It starts with being honest about what AI actually breaks. It's not just the volume hitting the QA step. It's the error profile. AI-generated code fails the way human code used to fail, just at higher volume: bad copy, UI subtly misaligned with the design, acceptance criteria quietly missed. Those are exactly the failures that need a person who knows what the screen was supposed to be — which, per everything above, is the one thing you can't hand back to a model. That's extra review work sitting on top of the extra volume, not instead of it.

So the shift I'd actually build toward has three pieces.

**Automation gets authored differently.** Not written for manual execution first and automated later as an afterthought, but built against the design and the implementation together from the start. Test plans align test cases to automation from the beginning: more technical suites, covering real surface area — personas, happy path, empty state, error state — but mapped to actual user journeys instead of a combinatorial blow-up. Five pathways into a feature and five user actions should produce five UI tests that cover the real journeys, not twenty-five tests covering every combination nobody actually hits. Choosing *which* five is the entire value, and that choice is a human judgment about who uses this app and how — precisely the judgment a model has no way to make. That's touch point one. If you're leaning towards 25 tests, you're using automated tests to guard against an architecture failure.
